...
const RegisterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (userType === "Student") {
        const response = await axios.post("http://localhost:5001/api/student_register", studentData)
        if (response.data) {
        ...
        const response = await axios.post("http://localhost:5001/api/register_teachers", teacherData)
