...
const AdminDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        const [teacherRes, contactRes] = await Promise.all([
          axios.get("http://localhost:5001/api/admin/teacher_requests"),
          axios.get("http://localhost:5001/api/from_contact_form"),
        ])
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      const endpoint =
        action === "approve"
          ? `http://localhost:5001/api/admin/teachers/${id}/approve`
          : `http://localhost:5001/api/admin/teachers/${id}/reject`
