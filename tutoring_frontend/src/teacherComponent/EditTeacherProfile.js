...
const EditTeacherProfile = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5001/api/teachers/${id}`, {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    axios
      .post(`http://localhost:5001/api/teacher_update/${id}`, teacherData, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
  ...
  const handleCredentialChange = async (e) => {
    ...
    try {
      const res = await axios.post(`http://localhost:5001/api/tutor_cred_change/${id}`, credentials, {
        headers: { Authorization: `Bearer ${token}` },
      })
